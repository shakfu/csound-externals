...
{
public:
	MidiEvent(int time, const t_csbyte *buf, int bytes);
	~MidiEvent() {}
	...
	t_csbyte m_buffer[MAX_MIDI_MESSAGE_SIZE];
	int m_size;
};
...
public:
	...
	bool AddMIDIEvent(t_csbyte *buf, int nBytes, bool lock); // Returns true on success.
	...
	inline void UpdateCtrlMatrix(t_csbyte chan, t_csbyte ctrl, t_csbyte val) { m_ctrlMatrix[chan][ctrl] = val; }
...
private:
	...
	t_csbyte m_ctrlMatrix[16][128];
	t_csbyte m_activeNoteMatrix[16][128];
};
