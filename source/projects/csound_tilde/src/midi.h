...
public:
	...
	int DequeueCompleteMessage(t_csbyte* buf, int n);
	...
	/* When m_sysex == true, any data bytes recieved through this function
       ...
       is set to false. 
	   Locks/Unlocks m_lock. */
	void Enqueue(t_csbyte b, bool lock = true);

	/* Enqueue n bytes from b into m_buffer.  b should contain only 1 complete MIDI message. 
	   Locks/Unlocks m_lock. */
	void EnqueueBuffer(t_csbyte* b, int n, bool lock = true);
...
private:
	...
	void DequeueBuffer(t_csbyte* b, int n); 

	/* Dequeue a single byte from the buffer. 
	   Does not lock/unlock m_lock.  Does not check m_buffer's size. */
	t_csbyte Dequeue();

	boost::circular_buffer<t_csbyte> m_buffer; // Contains the MIDI bytes.
