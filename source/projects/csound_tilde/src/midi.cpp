...
t_csbyte MidiBuffer::Dequeue()
{
	t_csbyte b = m_buffer.front();
	m_buffer.pop_front();
	return b;
}

void MidiBuffer::DequeueBuffer(t_csbyte* b, int n)
{		
	for(int i=0; i<n; i++)
	{
		b[i] = m_buffer.front();
...
int MidiBuffer::DequeueCompleteMessage(t_csbyte* buf, int n)
{
	t_csbyte b, status;
	ScopedLock lock(m_lock);
...
void MidiBuffer::Enqueue(t_csbyte b, bool lock)
{
...
void MidiBuffer::EnqueueBuffer(t_csbyte* b, int n, bool lock)
{
	t_csbyte status = 0;
...
void MidiBuffer::Flush()
{
	...
	for(t_csbyte c=0; c<16; c++)
		for(t_csbyte p=0; p<128; p++)
