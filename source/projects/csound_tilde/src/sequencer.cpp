...
MidiEvent::MidiEvent(int time, const t_csbyte *buf, int bytes) :
...
void Sequencer::StopRecording()
{
	...
	t_csbyte activeNoteMatrix[16][128];
	t_csbyte status = 0, chan, pitch, vel, b[3];
...
bool Sequencer::AddMIDIEvent(t_csbyte *buf, int nBytes, bool lock)
{
	ScopedLock k(m_lock,lock);
...
{
	t_csbyte status, chan, pitch, vel;
	ScopedLock k(m_lock);
...
{
	FILE *fp = NULL;
	t_csbyte *buffer = NULL, *bytePtr = NULL;
	...
	reverseBytes((t_csbyte*)&magic_number_reverse, sizeof(int));
	...
	buffer = (t_csbyte*) MemoryNew(fileSize);
	...
	else
	{
		...
		reverseNumber((t_csbyte*)&numEvents, sizeof(int), reverse);
		bytePtr += sizeof(int);

		for(i=0; i<numEvents; i++)
		{
			type = *(int*)bytePtr;
			reverseNumber((t_csbyte*)&type, sizeof(int), reverse);
			bytePtr += sizeof(int);

			time = *(int*)bytePtr;
			reverseNumber((t_csbyte*)&time, sizeof(int), reverse);
			bytePtr += sizeof(int);

			len = *(int*)bytePtr; // data1 size
			reverseNumber((t_csbyte*)&len, sizeof(int), reverse);
			bytePtr += sizeof(int);
			...
			case EVENT_TYPE_CONTROL:
				...
				reverseNumber((t_csbyte*)&value, sizeof(int), reverse);
			...
			case EVENT_TYPE_MIDI:
				...
				bytePtr += len + sizeof(int);	// skip data1 and data2 size (== 0 in this case)
				AddMIDIEvent((t_csbyte*) buf, len, false);
				break;
			}
		}
...
void Sequencer::WriteBinary(const string & file)
{
	...
	buffer = (t_csbyte *) MemoryNew(bufferSize);
